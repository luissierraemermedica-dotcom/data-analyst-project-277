...
FROM Sales AS s
    ...
    -- Relaciona cada venta con el vendedor
    INNER JOIN Employees AS e
        ON s.sales_person_id = e.employee_id

    -- Relaciona cada venta con el producto
    INNER JOIN Products AS p
        ON s.product_id = p.product_id

-- Agrupa las ventas por vendedor y día de la semana
GROUP BY
    e.employee_id,
    e.first_name,
    e.last_name,
...
-- Orden cronológico de lunes (1) a domingo (7),
-- y posteriormente por nombre del vendedor
ORDER BY
    EXTRACT(ISODOW FROM s.sale_date),
    seller;
...
SELECT
    -- Clasifica a los clientes según su edad
    CASE
    ...
    -- Cuenta la cantidad de clientes en cada grupo
    COUNT(*) AS age_count

FROM Customers

-- Solo considera clientes con edad de 16 años o más
WHERE age >= 16

-- Agrupa los clientes según la categoría de edad
GROUP BY
    CASE
        WHEN age BETWEEN 16 AND 25 THEN '16-25'
        WHEN age BETWEEN 26 AND 40 THEN '26-40'
...
-- Ordena los resultados según el orden lógico de los rangos
ORDER BY
    CASE
        WHEN
            CASE
